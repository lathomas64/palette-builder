<div class="Shadow_Controls Grid_Container">
	<div class="Arrow Up_Arrow Row Justify_Content_Center">
		<div class="Icon_Container"><?php include "../assets/01_Icons/pan-controls/arrow-up.svg"; ?></div>
	</div>
<<<<<<< HEAD
	<div class="Arrow Left_Arrow Justify_Content_Center Column">
		<div class="Icon_Container"><?php include "../assets/01_Icons/pan-controls/arrow-left.svg"; ?>
=======
	<div class="Row Space_Between Align_Items_Center">
		<div class="Column Arrow Left_Arrow">
			<div class="Icon_Container"><?php include "../assets/01_Icons/pan-controls/arrow-left.svg"; ?>
			</div>
		</div>
		<div class="Column Shadow_Name">{{shadow.name}}</div>
		<div class="Column Arrow Right_Arrow">
			<div class="Icon_Container"><?php include "../assets/01_Icons/pan-controls/arrow-right.svg"; ?>
			</div>
>>>>>>> ce5c530fc3c0c99056a9308fbe321922de0e0600
		</div>
	</div>
	<div class="Shadow_Name_Container Column Justify_Content_Center Align_Items_Center">
		<div class="Shadow_Name">Reign of Terra</div>
</div>
	<div class="Arrow Right_Arrow Justify_Content_Center Column">
		<div class="Icon_Container"><?php include "../assets/01_Icons/pan-controls/arrow-right.svg"; ?>
		</div>
	</div>
	<div class="Arrow Place_Aside Justify_Content_Center Column" style="display:none;">
		<div class="Icon_Container"><?php include "../assets/01_Icons/pan-controls/arrow-left.svg"; ?>
	</div>
	</div>
	<div class="Arrow Down_Arrow Justify_Content_Center Row"><div class="Icon_Container"><?php include "../assets/01_Icons/pan-controls/arrow-down.svg"; ?>
	</div></div>
		<div class="Arrow Remove Justify_Content_Center Column">
			<div class="Icon_Container"><?php include "../assets/01_Icons/pan-controls/trash.svg"; ?></div>
		</div>
</div>