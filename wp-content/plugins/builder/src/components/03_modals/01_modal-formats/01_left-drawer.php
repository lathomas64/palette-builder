<div id="leftDrawer" class="Modal Fade_In">
	<div class="Row Reverse Space_Between Drawer_Overlay">
		<div class="">&nbsp;</div>
		<div class="Drawer_Container Fade_Out_Right">
	 	<div class="Close_Modal Row Reverse Justify_Content_Flex_Start">
				<button id="signinCloseBtn" class="Text_Button Icon_Button Row Reverse Gap_4 Align_Items_Center Close">
			  <div class="Icon_Container">
			   <svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8 7.05333L3.05533 4L8 0.94L6.47773 0L0 4L6.47773 8L8 7.05333Z"></path>
		   </svg>
				</div>
		  <span>go back</span>
			</button>
	 </div>
		 <div class="Panel_Content Column Gap_40">
		 	<?php include "components/03_modals/02_modal-content/01_left-drawer/log-in-content.php"; ?>	
			 	<?php include "components/03_modals/02_modal-content/01_left-drawer/register-content.php"; ?>	
			</div>
		</div>
	</div>
</div>
